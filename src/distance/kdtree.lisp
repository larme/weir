...
(defun -dst2 (verts i x y z)
  (declare #.*opt-settings*
           ...
           (double-float x y z))
  (+ (expt (- (the double-float (aref verts i)) x) 2)
     (expt (- (the double-float (aref verts (+ 1 i))) y) 2)
     (expt (- (the double-float (aref verts (+ 2 i))) z) 2)))
...
(defun rad (kd xy rad)
  ...
  (with-struct (kdtree- root verts) kd
    ...
    (let* ((x (vec:3vec-x xy))
      ...
      (-rad verts root x y z (expt rad 2) :res res)
      res)))
